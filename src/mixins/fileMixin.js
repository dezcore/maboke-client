...
export default {
    ...
    methods : {
        getFile : function(name, callBack) {
            if(name) {
              this.getData(import.meta.env.VITE_API_URL + "/google/drive/file/content/" + name, (response) => {
                if(callBack)
                  callBack(response)
              })
            }
        ...
        getFiles : function(callBack) {
            this.getData(import.meta.env.VITE_API_URL + "/google/drive/files", (response) => {
              if(callBack)
                callBack(response)
            })
        },
        postFile : function(folderName, fileName, data, callBack) {
          if(fileName && data) {
            this.postData(import.meta.env.VITE_API_URL  + "/google/drive/", {
        ...
        deleteFile : function(file, callBack) {
            if(file) {
              this.deleteData(import.meta.env.VITE_API_URL + "/google/drive/file/" + file.fileId, () => {
                if(callBack)
                  callBack()
              })
            }
        ...
        downloadFile : function(parameters, abortController, callBack) {
          ...
          if(parameters) {
            this.getStream(import.meta.env.VITE_API_URL + "/google/youtube/download", parameters, abortController, (data) => {
        ...
        deleteFileByName : function(fileName, callBack) {
          if(fileName) {
            this.deleteData(import.meta.env.VITE_API_URL + "/google/drive/folder/" + fileName, () => {
              if(callBack)
                callBack()
            })
          }
